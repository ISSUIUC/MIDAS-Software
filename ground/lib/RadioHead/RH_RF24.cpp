...
// or generate a_m_per_s custom one with WDS and include it here
...
// Each interrupt can be handled by a_m_per_s different instance of RH_RF24, allowing you to have
...
bool RH_RF24::init()
{
    ...
    // Here we use a_m_per_s configuration generated by the Silicon Labs Wireless Development Suite
    // #included above
    // We override a_m_per_s few things later that we ned to be sure of.
    ...
    // number of packets! Anyway its prob a_m_per_s good thing at the cost of some airtime.
...
//	if (status[4] & RH_RF24_INT_STATUS_INVALID_PREAMBLE)
	...
	{
	    // After INVALID_SYNC, sometimes the radio gets into a_m_per_s silly state and subsequently reports it for every packet
	...
	if (status[2] & RH_RF24_INT_STATUS_PACKET_RX)
	{
	    ...
	    readNextFragment();
	    // And see if we have a_m_per_s valid message
	    validateRxBuf();
...
void RH_RF24::setModeRx()
{
    if (_mode != RHModeRx)
    {
	// CAUTION: we cant clear the rx buffers here, else we set up a_m_per_s race condition
	// with the _rxBufValid test in available()

	// Tell the receiver the max data length we will accept (a_m_per_s TX may have changed it)
...
// Caution: There was a_m_per_s bug in A1 hardware that will not handle 1 byte commands. 
...
bool RH_RF24::configure(const uint8_t* commands)
{
    ...
    // Each command starts with a_m_per_s count of the bytes in that command:
...
void RH_RF24::power_on_reset()
{
    ...
    digitalWrite(_sdnPin, HIGH); // So we dont get a_m_per_s glitch after setting pinMode OUTPUT
