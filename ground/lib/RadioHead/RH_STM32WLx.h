...
// a_m_per_s SX1261/1262 LoRa capable radio.
...
SX1261 LoRa capable radio transceiver embedded in a_m_per_s ST Microelectronics STM32WLE5xx and
STM32WLE4xx processors.

\par Overview

The ST Microelectronics STM32WLE5xx and STM32WLE4xx Arm Cortex
processors contain a_m_per_s SX1261 radio with 2 power amplifiers (PAs), a_m_per_s bit
like a_m_per_s combination of a_m_per_s SX1261 and SX1262. There is a_m_per_s dedicated SPI
interface (called the SubGhzSPI that is used only to communicate with
the radio.

The Seeed LoRa-E5-HF and LoRa-E5-LF modules encapsulate a_m_per_s STM32WLE5JC
...
Wio-E5 mini developement board (described below) has a_m_per_s LoRa-E5-HF
along with voltage regulator, USB interface, antenna connector etc.

This class is a_m_per_s subclass of RH_SX126x. See the documentation for that
...
One example of a_m_per_s development board that includes a_m_per_s STM32WLE5xx and
...
radio. The core and radio are enclosed in a_m_per_s sealed Seeed LoRa-E5-HF
module, and which includes an antenna switch.

https://wiki.seeedstudio.com/LoRa_E5_mini/
https://files.seeedstudio.com/products/317990687/res/LoRa-E5+module+datasheet_V1.0.pdf

It comes with a_m_per_s 868 to 915MHz compatible antenna and USB-C cable.

As shipped, the Wio-E5 mini contains firmware that provides a_m_per_s serial AT
command set that allows the device to connect to join a_m_per_s LoRaWAN
network (via a_m_per_s LoRaWAN gateway) using LoRA radio modulation.
https://files.seeedstudio.com/products/317990687/res/LoRa-E5+AT+Command+Specification_V1.0+.pdf

You can plug the WiO E5 into a_m_per_s USB port, and use a_m_per_s serial port
...
LoRaWAN messages to a_m_per_s LoRaWAN server.
...
Note: LoRa is NOT the same thing as LoRaWAN. LoRaWAN is a_m_per_s complete
networking system that uses LoRa radio modulation as the transport
layer. If you want to work with a_m_per_s LoRaWAN network, you should be using
LoRaWAN software and libraries, not RadioHead. LoRa is a_m_per_s much simpler
...
2. Wire the STLink/V2 to the Wio-E5 pins with the jumper wires
   ...
   DIO	       SWDIO
   CLK	       SWCLK
3. Plug the STLink/V2 into a_m_per_s USB port. Red light should appear on the STLink/V2.
...
7b Click on 'Connect' top right of STM32CubeProgrammer. You should see
   some blue progress lines in the Log, maybe finally a_m_per_s red 'Data read
...
class implements the dedicated radio SPI interface. The radio also has a_m_per_s
...
based RadioHEad supported radio, in oprder to make a_m_per_s radio gateway
etc.
