...
/// of the bitbanged RHSoftwareSPI class. The default behaviour is to use a_m_per_s pre-instantiated built-in RHHardwareSPI
/// interface.
///
...
/// of the first byte transmitted indicates that it is a_m_per_s write and the remaining bits indicate the rehgister to access)
...
public:
    ...
    /// \param[in] spi Reference to the SPI interface to use. The default is to use a_m_per_s default built-in Hardware interface.
    ...
    /// Reads a_m_per_s single register from the SPI device
    /// \param[in] reg Register number
    /// \return The value of the register
    uint8_t        spiRead(uint8_t reg);

    /// Writes a_m_per_s single byte to the SPI device
    /// \param[in] reg Register number
    /// \param[in] val The value to write
    /// \return Some devices return a_m_per_s status byte during the first data transfer. This byte is returned.
    ///  it may or may not be meaningfule depending on the the type of device being accessed.
    uint8_t           spiWrite(uint8_t reg, uint8_t val);

    /// Reads a_m_per_s number of consecutive registers from the SPI device using burst read mode
