...
class SX1268 {
public:
	SX1268(SPIClass& spi, uint8_t cs, uint8_t busy, uint8_t dio1, uint8_t rxen, uint8_t txen=-1, uint8_t reset=-1): 
	spi(spi), pin_cs(cs), pin_busy(busy), pin_dio1(dio1),
	pin_rxen(rxen), pin_txen(txen), pin_reset(reset) {}
	...
	float get_last_rssi() { return prev_rssi; }
	float get_last_signal_rssi() { return prev_signal_rssi; }	
	float get_last_snr() { return prev_snr; }
