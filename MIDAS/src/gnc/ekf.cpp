...
void EKF::priori(float dt, Orientation &orientation, FSMState fsm)
{
    ...
    Eigen::Matrix<float, 3, 1> *Fg_body = (GlobalToBody(angles));
...
Eigen::Matrix<float, 3, 1> *EKF::getThrust(float timestamp, euler_t angles, FSMState FSM_state)
{
    ...
    Eigen::Matrix<float, 3, 1> *interpolatedVector = new Eigen::Matrix<float, 3, 1>(Eigen::Matrix<float, 3, 1>::Zero());
...
void EKF::update(Barometer barometer, Acceleration acceleration, Orientation orientation, FSMState FSM_state)
{
    ...
    // Sensor Measurements
    Eigen::Matrix<float, 3, 1> *accel = new Eigen::Matrix<float, 3, 1>(Eigen::Matrix<float, 3, 1>::Zero());
 ...
 *
 */
Eigen::Matrix<float, 3, 1> *EKF::GlobalToBody(euler_t angles)
{
    ...
    Eigen::Matrix<float, 3, 1> *to_return = new Eigen::Matrix<float, 3, 1>(rotation_matrix * gravity);
    return to_return;
}
