...
RocketSystems<EmulatedHw>* create_and_start(EmulatedHw& source, const char* sink_name) {
    SDSink* sink = new SDSink(sink_name);
    RocketSystems<EmulatedHw>* systems = new RocketSystems(source, *sink);

    uint8_t* stack = new uint8_t[4096];
    xTaskCreateStaticPinnedToCore((TaskFunction_t) [](void* arg) { ((RocketSystems<EmulatedHw>*) arg)->begin(); }, sink_name, 4096, systems, tskIDLE_PRIORITY, stack, nullptr, 0);
    return systems;
}
...
//    RocketSystems<EmulatedHw>* stage1_systems = create_and_start(together_sim, "stage1_sink.launch");
//    RocketSystems<EmulatedHw>* stage2_systems = create_and_start(together_sim, "stage2_sink.launch");
//
