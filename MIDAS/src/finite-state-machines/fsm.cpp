...
StateEstimate::StateEstimate(RocketData& state) {
    ...
    altitude = sensor_average<BarometerData, 8>(state.barometer, [](BarometerData& data) {
        return (double) data.altitude;
    });
    jerk = sensor_derivative<HighGData, 8>(state.high_g, [](HighGData& data) {
        return (double) data.ax;
    });
    vertical_speed = sensor_derivative<BarometerData, 8>(state.barometer, [](BarometerData& data) {
        return (double) data.altitude;
    });
}
