...
BASE_TYPES: dict[str, Type] = {
    ...
    'uint8_t': Integer(1),
    'uint16_t': Integer(2, signed=False),
    'uint32_t': Integer(4, signed=False),
    'size_t': Integer(4, signed=False),
    'uint64_t': Integer(8, signed=False),
    'int16_t': Integer(2, signed=True),
...
ASSOCIATE_RE = re.compile(r'ASSOCIATE\(\s*(\w+)\s*,\s*(\w+)\s*,\s*(\w+)\s*\)')
...
class Preprocessor:
    ...
    def include_file(self, path: Path) -> str:
        ...
        if path not in self.included_files:
            self.included_files[path] = path.read_text()
    ...
    def preprocess(self, file_path: str, text: str) -> str:
        ...
        for i, line in enumerate(text.splitlines()):
            ...
            if m:
                ...
                self.associations[field_name] = disc_id
                processed += "\n"
                continue
...
def parse_file(file: Path) -> tuple[Context, dict[str, str]]:
...
class TypeID(IntEnum):
    """These IDs get baked into the log file header so the decoder knows what each field is."""
    STRUCT = 0
    FLOAT32 = 1
    FLOAT64 = 2
    INT8 = 3
...
_INT_IDS = {
    (1, True): TypeID.INT8,    (1, False): TypeID.UINT8,
    (2, True): TypeID.INT16,   (2, False): TypeID.UINT16,
    (4, True): TypeID.INT32,   (4, False): TypeID.UINT32,
    (8, True): TypeID.INT64,   (8, False): TypeID.UINT64,
}

def type_id_of(t: Type) -> TypeID:
    match t:
        case Boolean(): return TypeID.BOOL
        case Float(size=4): return TypeID.FLOAT32
        case Float(): return TypeID.FLOAT64
        case Integer(): return _INT_IDS[(t.size, t.signed)]
        case Enum(): return TypeID.ENUM
        case Struct(): return TypeID.STRUCT
        case Union(): return TypeID.UNION
        case Array(): return TypeID.ARRAY
    raise Exception(f"can't map {t} to a TypeID")


def flatten(name: str, t: Type) -> list[tuple[str, TypeID, int]]:
    """Walk the type tree in pre-order, returning (name, type_id, count) tuples.
    count = number of children (struct fields / union variants) or array length. 0 for leaves."""
    tid = type_id_of(t)

    if isinstance(t, Struct):
        kids = [e for fn, ft in t.members.items() for e in flatten(fn, ft)]
        return [(name, tid, len(t.members))] + kids
    elif isinstance(t, Union):
        kids = [e for vn, vt in t.variants.items() for e in flatten(vn, vt)]
        return [(name, tid, len(t.variants))] + kids
    elif isinstance(t, Array):
        return [(name, tid, t.count)] + flatten("_", t.item)
    else:
        return [(name, tid, 0)]


def build_variant_table(entries, associations, disc_enum):
    """Maps discriminant int values to their index in the flattened entries list."""
    disc_to_val = {name: val for val, name in disc_enum.variants.items()}
    table = []
    for i, (name, _, _) in enumerate(entries):
        if name in associations:
            table.append((disc_to_val[associations[name]], i))
    table.sort()
    return table
